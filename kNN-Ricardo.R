...
kNN <- function(train, test, tr.class, te.class, dist = 'euclidean', k = 3, lambda)
{
    ...
    observed_agreement  <- acc
    expected_agreement  <- sum(rowSums(conf_matrix) * colSums(conf_matrix)) / (n^2)
   
    kappa               <- (observed_agreement - expected_agreement) / (1 - expected_agreement)
    se_kappa            <- sqrt((observed_agreement * (1 - observed_agreement)) / (n*(1 - expected_agreement)^2))
